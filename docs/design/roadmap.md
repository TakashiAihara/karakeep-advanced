...
| Milestone | issue |
|---|---|
| M0 — Quality foundation | #17 #18 #19 #21 #22 #23 #24 #42 #47 |
| M1 — Save reliability | #25 #26 #27 #28 #29 #30 #31 #44 |
| M2 — Server-side metadata | #32 #33 #35 #36 #45 |
| M3 — Manager page | #34 #37 #38 #39 #46 |
| M4 — Lifecycle | #40 #41 #48 #49 #50 #51 #52 #53 |

## 設計パネルで足された論点 (2026-08-28)

M0/M1/M2 を実装したあと、5 つの角度で独立に設計パネルを回し、各案を反対尋問にかけた。
出てきたもののうち roadmap を変えたものだけ挙げる。詳細は各 issue と `decisions.md` の D07-D10。

### 規模で先に壊れるのは容量ではなく順序

10,000 グループでもキャッシュは 2.5 MB で、`storage.local` の枠には収まる。
先に壊れるのは **`recentGroupIds` (上限 50) が唯一の順序機構であること**で、
これは 5 年後ではなく 1 年目に効く。→ D09 / #45

### アーカイブは転送量を減らさない

`GET /lists` にフィルタが無いので、親を付け替えたグループも同じレスポンスに乗って返り続ける。
M4 の効果は UI から隠れることだけで、規模対策にはならない。→ #51

### Archived を見る画面が無いまま auto-archive を出せない

`orderGroups` は `parentId` の厳密一致で絞るので、
アーカイブした瞬間にグループは拡張のどこからも見えなくなり、消えたのと区別がつかない。→ #50

### 姉妹拡張は Highlight 1 本

Omnibox は新しい permission を要さないので、別拡張にする理由が無い。→ D07 / #47

### smart list は機能ではなく spike

`query` の文法がスキーマに無いので、動くかどうかを確かめるまで設計しない。→ #49
